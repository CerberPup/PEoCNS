...
cHistogram* UdpBasicBurst::hist = nullptr;
...
void UdpBasicBurst::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        if (!hist)
            hist = new cHistogram("Default", new cDefaultHistogramStrategy());
        hist->setName("czasy");
        //hist.setRange(0,0.5);
        hist->setNumBinsHint(100);
    }
}
...
Packet *UdpBasicBurst::createPacket()
{
    ...
    Packet *pk = new Packet("---shouldhavename---");
...
void UdpBasicBurst::ackPacket(Packet *msg)
{
    long msgByteLength = *messageLengthPar;
    Packet *pk = new Packet("ACK");
    const auto& payload = makeShared<ApplicationPacket>();
    payload->setChunkLength(B(msgByteLength));
    payload->setSequenceNumber(numSent);
    payload->addTag<CreationTimeTag>()->setCreationTime(simTime());
    ...
    auto ports = msg->getTag<L4PortInd>();
    if(dontFragment)
        pk->addTag<FragmentationReq>()->setDontFragment(true);
    pk->setTimestamp();
    emit(packetSentSignal, pk);
    socket.sendTo(pk, l3Addresses->getSrcAddress(), ports->getSrcPort());

    /*auto tst = msg->peekDataAt<Ipv4Header>(b(5), b(20));
    msg->popAtFront(b(5));
    auto ipv4 = msg->popAtFront<Ipv4Header>();*/

   /* Packet *packet = new Packet("ACK");
    //std::cout<< ipv4->getSourceAddress();
    //L3Address destAddr(ipv4->getSourceAddress());
    emit(packetSentSignal, packet);
    socket.sendTo(packet, l3Addresses->getSrcAddress(), ports->getSrcPort());
    numSent++;*/
}
...
void UdpBasicBurst::processPacket(Packet *pk)
{
    ...
    try
    {
        double creationtime = std::stod(pk->getName());
        hist->collect(creationtime - SimTime());
        ackPacket(pk);
    }
...
void UdpBasicBurst::generateBurst()
{
    ...
    payload->setName(std::to_string(payload->getCreationTime().dbl()).c_str());
    if(dontFragment)
